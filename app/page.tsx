...
function SigninScreen({ onNext, onSignup, isLoading, onLogoClick, isDesktop }: { onNext: (email: string, password: string) => Promise<void>; onSignup: () => void; isLoading: boolean; onLogoClick: () => void; isDesktop: boolean }) {
  ...
  const handleGoogleSignin = async (credentialResponse: any) => {
    ...
    try {
      ...
      // Store the token and redirect (handled by parent app)
      const event = new CustomEvent('googleSigninSuccess', { detail: data });
      ...
      <div className="fadeUp" style={{ paddingTop: 16, maxWidth: isDesktop ? 440 : undefined, margin: isDesktop ? "0 auto" : undefined, ...(isDesktop ? Card({ padding: "36px 32px", marginTop: 20 }) : {}) }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <button
            ...
            style={{
              ...Lime({ width: "100%", padding: "12px", opacity: googleLoading ? 0.5 : 1 }),
              display: "flex",
              alignItems: "center",
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" style={{ display: "inline" }}>
              <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/>
...
function SettingsScreen({ user, onBack, bgMode, toggleBgMode, notificationsEnabled, sendTestNotification, onLogoClick, notificationTime, setNotificationTime, notificationEnabled: notificationScheduleEnabled, toggleNotificationSchedule, scheduleDailyReminder, isDesktop, onRequestNotifications }: { user: { name: string; email: string; whatsapp: string }; onBack: () => void; bgMode: "white" | "black"; toggleBgMode: () => void; notificationsEnabled: boolean; sendTestNotification: () => void; onLogoClick: () => void; notificationTime: string; setNotificationTime: (time: string) => void; notificationEnabled: boolean; toggleNotificationSchedule: (enabled: boolean) => void; scheduleDailyReminder: () => void; isDesktop: boolean; onRequestNotifications: () => void }) {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div onClick={() => { if (!notificationsEnabled) onRequestNotifications(); }} style={{ width: 44, height: 24, borderRadius: 12, background: notificationsEnabled ? C.lime : C.border, position: "relative", cursor: "pointer" }}>
